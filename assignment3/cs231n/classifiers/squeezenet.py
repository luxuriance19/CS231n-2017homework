...
def fire_module(x,inp,sp,e11p,e33p):
    with tf.variable_scope("fire"):
        with tf.variable_scope("squeeze"): # x [batch, in_height, in_width, in_channels]
            W = tf.get_variable("weights",shape=[1,1,inp,sp]) # [filter_height,filter,_width, in_channels, out_channels]
...
class SqueezeNet(object):
    def extract_features(self, input=None, reuse=True):
        ...
        layers = []
        '''
        conv (3*3*64, stride=2) -> relu -> max_pool(3*3, stride=2) -> 2 * fire_modules -> max_pool(3*3, stride=2) 
        -> 2 * fire_modules -> max_pool(3*3, stride=2) -> 4 * fire_modules
        '''
